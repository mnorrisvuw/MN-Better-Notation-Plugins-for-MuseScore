...
MuseScore {
	...
	function checkIllegalDurations (noteRest) {
		if (displayDur === minim * 2.25 && timeSigStr === '7/8') {
			if (isNote) {
				addError ("It is better to write divide a double-dotted minim in 7/8 time,\ninto a dotted crotchet and a minim (or vice versa)\nto match the underlying beat divisions.", noteRest);
				return;
			}
		}
		if (timeSigStr === '5/8' || timeSigStr === '7/8') {
			if (displayDur === minim) {
				if (isNote) {
					if (timeSigStr === '7/8') {
			...
			if (isInTuplet) {
				...
				if (endBeat != startBeat) {
					...
					if (startBeatOffset != 0) {
						addError ("Offbeat tuplets that cross beats can be difficult\nto read and are not recommended.\nEither split up the tuplet, or consider rewriting.",theTuplet);
					} else {
		...
		if (simplificationFound) {
			...
			if (tiedNotes.length == 2 && canCheckThisBar) {
				...
				if (note1.tuplet == null && note2.tuplet == null) {
					...
					var startBeat = Math.trunc(startPos/beatLength);
					checkTiedPair (note1,note2,startBeat,startFrac);
				}
			}
		}
	}
	...
	function checkTiedPair (note1,note2,startBeat,startFrac) {
